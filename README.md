...
- Phase 8G.3.2 temporarily makes `/demo` the public live customer-facing example for remote feedback while `/support` remains authenticated and engineering-oriented. Public/customer cards omit RMI/raw/formula, sensor identity, provenance, paired-source, failure-counter, and diagnostics detail. The public page has no manual refresh, polls every 15 visible minutes, and hard-limits history to 24 hours while labeling longer disabled choices `customer only`; deterministic curated Demo data remains planned for Phase 8I.
