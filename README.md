...
- Phase 8B.5 Gen2 Endpoint Integration and Closeout is FRONTEND VALIDATED / COMPLETE PENDING COMMIT AND PUSH; production deployment validation remains pending.
- Phase 8B.5 adds deterministic Garden Readings, independent Garden Reading Quality and MBG Diagnostics surfaces, a compatible-family trend chart, and responsive hosted cleanup while preserving read-only and local watering-control boundaries. Detailed evidence is recorded in [`docs/product/phase8b-gen2-endpoint-contract-cleanup.md`](./docs/product/phase8b-gen2-endpoint-contract-cleanup.md).
