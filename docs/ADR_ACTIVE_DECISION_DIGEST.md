...
Phase 8C.5A is the approved environmental-presentation design, and Phase 8C.5B is implemented and locally validated. Exact condition ranges drive measurement-specific card colors and compact full-scale footer pills. Ordinary current cards assume currency and show concise condition wording in the upper-right pill; non-current/fallback/fault cards reserve that pill for evidence state. Last Good and Last Reliable evidence retains condition/color through the inclusive 50-minute Phase 8C.3 boundary; older evidence is neutral and pill escalation remains unchanged. Light uses a subdued brightness scale, moisture uses dry-tan through sage/teal to wet-blue rather than pass/fail red/green, and reservoir remains binary red/green. No query, SQL, storage, polling, firmware, device, or watering authority changed. Feels Like, Dew Point, customer adoption, deterministic Demo, Gen1 review, major UI modernization, and watering-event/threshold work remain separate unnumbered prioritization candidates.
