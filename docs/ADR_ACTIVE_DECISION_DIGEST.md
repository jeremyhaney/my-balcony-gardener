...
Phase 8C.1 defines how later capability-driven cards will join currently commissioned capabilities to the best available measurement state; no frontend consumer exists yet. Commissioned sensors must remain visible when stale, invalid, missing, unavailable, absent from the latest batch, or not yet reported. Stable generic labels, units, formatting, charts, and visual styling remain frontend responsibilities; installation-specific friendly/location names may accompany provisioning. Authorization-specific loaders will normalize into a common logical-sensor model without turning Gen1 or Gen2 storage into configuration. The future Phase 8C.2 is the authenticated Support Vertical Slice; customer adoption waits for an authorized customer-visible commissioned device.
