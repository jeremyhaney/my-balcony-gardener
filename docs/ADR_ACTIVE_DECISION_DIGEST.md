...
- `sensor_logs`: obsolete live compatibility table with no supported consumer; Phase 8F.10 retirement proposal only, not yet executed.
- `sensor_events`: isolated manual operational context, not device-originated telemetry and not command/control; retain the table, while Phase 8F.10 proposes deleting its three sample rows and revoking unused API grants.
...
The approved post-8C.5 sequence is Phase 8D Watering-Event Visibility Restoration, Phase 8E Feels Like and Dew Point, Phase 8F Gen1 Risk Review/Containment/Retirement, Phase 8G Watering-Threshold Presentation, Phase 8H Customer Adoption and Customer-Led UI Modernization, and Phase 8I Deterministic Demo. Phase 8D is regression repair; Phase 8E is a bounded display-only microclimate slice; Phase 8F must progress from evidence inventory to tested containment/removal; Phase 8G remains read-only; Phase 8H must use authorized customer visibility; and Phase 8I waits until customer/UI direction stabilizes. Numbering establishes priority but grants no implementation authority. See [`docs/product/phase8-post-8c5-priority-sequence.md`](./product/phase8-post-8c5-priority-sequence.md).
