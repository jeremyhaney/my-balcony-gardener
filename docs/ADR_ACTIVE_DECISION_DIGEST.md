...
Phase 8G.3 adds `Prototype02` / `prototype02-gen2` as a new explicit supported bench identity without restoring Prototype01/Bench01. P02 is controller-path watering capable through a relay/LED simulation but has no pump and no physical water delivery; capability and event evidence state that distinction explicitly. It preserves ADR 0025's 30/60-second local programs, cancellation, WL01 start/loss interlocks, fail-off ordering, and network exclusion while active. The commissioned unit is Support-only, customer-hidden, and Demo-hidden. Runtime proof passed with all seven configured measurement records, paired simulated cycles, blocked starts, active-run safety cutoffs, telemetry/heartbeat, and endpoint recovery; the deployed authenticated Support presentation was subsequently confirmed. No new ADR is required because Phase 8G.3 applies existing Gen2, local-control, and identity boundaries. See [`docs/product/phase8g3-prototype02-firmware-update-and-commissioning.md`](./product/phase8g3-prototype02-firmware-update-and-commissioning.md).
