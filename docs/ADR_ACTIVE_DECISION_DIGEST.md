...
Phase 8C.1 defines how capability-driven cards join currently commissioned capabilities to the best available measurement state. The Phase 8C.2 authenticated Support consumer is implementation-complete and locally validated; post-push production validation remains pending. It reads the protected Support view, normalizes one object per logical sensor, filters current-effective lifecycle rows, caches once per device in memory, and drives cards/history through exact logical-series mappings without measurement inventory fallback. Commissioned sensors retain last-good evidence when stale, invalid, missing, unavailable, absent from the latest batch, or not yet reported. Jeremy's 2026-08-16 review confirmed Balcony02's eleven-card presentation and the correct zero-capability state for unprovisioned devices. Prototype01 firmware/provisioning and customer adoption remain later work.
