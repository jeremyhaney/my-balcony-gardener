...
- Phase 8C.1 documents the approved frontend application in [`docs/product/phase8c1-hosted-frontend-capability-integration-design.md`](./product/phase8c1-hosted-frontend-capability-integration-design.md). Phase 8C.2 is production-validated (**Pass**) and operationally closed for authenticated Support: it reads `support_device_capabilities`, normalizes to a shared logical-sensor model, filters current-effective rows, caches per device in memory, applies exact logical-series mappings, and uses no inventory fallback. Jeremy's 2026-08-16 production smoke test confirmed Balcony02's eleven approved cards in Light, Air, Water, Soil order, Prototype01's successful-zero state, and preserved Demo availability; unforced failure, lifecycle, cache, refresh, polling, and concurrency cases remain test/inspection-supported. Customer adoption, Prototype01 firmware/provisioning, evidence/quality refinement, Gen1 cleanup, deterministic Demo work, visual modernization, and public capability access remain deferred. Phase 8D remains Local Schedule UI and Schedule Persistence.
- Phase 8C.3 refines authenticated Support evidence interpretation without changing authority or data access. `measured_at` owns freshness; `batch_created_at` remains transport evidence; current thresholds are 50 minutes and 95 minutes with device-active qualification. Last-good, invalidity, latest-package omission, selected-window absence, derived availability, environmental condition, evidence-health severity, and device reporting remain separate frontend concepts. Support health expectations come from commissioned capability descriptors. Bounded consecutive counts use already-fetched rows, and no additional query, polling, storage, SQL, Supabase, firmware, device, watering, or Demo capability path is introduced. RMI remains derived from its exact raw ADC source; engineering details are Support-only. Authenticated production validation passed on 2026-08-17 and the slice is operationally closed. See [`docs/product/phase8c3-commissioned-measurement-evidence-state-and-health-refinement.md`](./product/phase8c3-commissioned-measurement-evidence-state-and-health-refinement.md).
- Phase 8C.4 adds deterministic validation evidence for the existing derived-unavailable RMI branch and for the separation between fault labels and environmental-condition currency. It changes no application runtime or architecture boundary. Validation passed 22/22 tests, lint, build, and `git diff --check`; commit `de7e5de8bcc9e1d73bc72333939225210b273843` is complete on `main`. See [`docs/product/phase8c4-deterministic-support-evidence-fault-state-validation.md`](./product/phase8c4-deterministic-support-evidence-fault-state-validation.md).
- Phase 8C.5A defines measurement-quality boundaries without implementing them. Stored Gen2 observations and device-reported `valid`, `quality`, and `reason` remain append-only evidence; commissioned capability, freshness, environmental interpretation, Support concern, future customer display eligibility, and firmware watering/control eligibility remain separate. Generic raw-ADC trust handling from the failed prior moisture path is obsolete, has no compatibility requirement, and must be removed rather than propagated. Future electrical rules must be designed directly for the ADS1115-backed SEN0308 provider/system, not selected by the generic name `raw_adc`; the inactive `hostedMeasurementTrust.ts` is not Gen2 product authority. The preferred later presentation gate uses already-fetched rows and adds no query, SQL, storage mutation, polling, or Disk IO. See [`docs/product/phase8c5a-measurement-quality-gates-discovery-and-boundary-design.md`](./product/phase8c5a-measurement-quality-gates-discovery-and-boundary-design.md).
- Phase 8C.5B implements the frontend presentation gate in `measurementPresentationEligibility.ts`. Exact commissioned identities select product/provider rules; air uses `0..130 °F`, soil uses `10..130 °F`, and SEN0562 accepts `0..65535 lux` while recording the upper measurement ceiling as a non-rejecting concern. Latest rejected evidence remains visible to authenticated Support, ordinary value/condition and trend paths use only presentation-eligible rows, and `lastPresentationEligibleRow` remains separate from device-good evidence. No query, polling, SQL, storage, firmware, or control path changes.
