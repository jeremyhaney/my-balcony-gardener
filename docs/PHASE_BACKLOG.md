...
- **Highest executable priority:** create the lightweight brand foundation and first business card. The watering-event graph visibility repair is complete and locally accepted, with phone-sized and post-push hosted validation retained as follow-ups.
...
- **Then:** proceed into Phase 8H customer adoption and customer-led UI modernization, followed by Phase 8I deterministic Demo.
...
| Layer | Current meaning | Phase implication |
| --- | --- | --- |
| Working Gen2 product | Locally controlled watering hardware with firmware-owned safety, commissioned sensors, append-only evidence, and authenticated read-only Support visibility. | Complete the evidence and presentation path without introducing hosted command/control or new automatic watering authority. |
| Current Phase 8 direction | Improve capability truth, measurement quality, evidence health, gardener-facing interpretation, and Support presentation. | Sensor readings become more trustworthy and useful while remaining evidence and display unless separately authorized. |
| Field learning | Collect data, map balcony microclimates, learn dry-down, and compare readings with human judgment. | Balcony02 and Prototype01 evidence should teach the project what sensor readings mean across real balcony conditions. |
...
- **Phase 8D Watering-Event Visibility Restoration is complete and hosted-validated.** Commit `406bce9` restored normal and safety-terminal cycles, stored start timestamps, honest sub-second wording, consistent type/color presentation, and count-preserving in-window chart markers without changing queries, storage, firmware, or watering authority. Jeremy confirmed all seven production-evidence cycles on the authenticated hosted interface on 2026-08-19. See the [closeout](./product/phase8d-watering-event-visibility-restoration-closeout.md).
- **Phase 8F.1 begins the approved Gen1 risk retirement sequence with a bounded frontend removal.** The unsupported Phase 7C Prototype01 panel, its hardcoded polling, dedicated presentation, stale local response types, and exclusively owned request helpers are retired while shared local control, `/logs`, device registry, firmware, hosted Gen2 behavior, and Demo behavior remain in place. See the [implementation record](./product/phase8f1-retire-unsupported-live-measurements-frontend.md).
- **Phase 8F.2 retires the remaining direct browser-to-device frontend.** `LiveStats`, selected-target five-second `/logs` polling, browser identity gating, local Water Now, and the exclusively owned local-control target definitions are removed. The shared device registry and `sensor_logs` history path remain, and firmware endpoints and watering authority are unchanged. See the [implementation record](./product/phase8f2-retire-livestats-local-water-now-frontend.md).
- **Phase 8F.3 retires the local route and frontend legacy history path.** Ordinary and hosted builds use the same hosted Gen2 shell; Demo no longer requests `sensor_logs`; frontend legacy history types, adapter/state, chart, and health presentation are removed. Firmware posting/endpoints/profiles, Supabase storage/policies, hosted Gen2 queries/presentation, and registry entries remain. See the [implementation record](./product/phase8f3-retire-local-dashboard-sensor-logs-frontend.md).
- **Phase 8F.4 retires obsolete firmware build profiles.** Balcony01, Scout01, and Prototype01/Bench01 environments and the retired Balcony01 default selection are removed. `balcony02-gen2` is the only supported selectable firmware environment; shared ESP32 mechanics remain non-selectable, and Balcony02 behavior/identity flags are unchanged. Historical evidence and Gen1 implementation branches remain. See the [implementation record](./product/phase8f4-retire-obsolete-firmware-build-profiles.md).
- **Phase 8F.5 retires unreachable Gen1 firmware implementation.** Legacy `sensor_logs` writes, `/logs`, HTTP `/water-now`, DHT/direct-soil/VEML modules, runtime capability aggregation/scans, and excluded status/heartbeat/setup/loop sides are removed. Balcony02's static endpoint contracts, Gen2 ingestion, physical-button/reservoir safety, pump shutoff, and local control authority remain. See the [implementation record](./product/phase8f5-retire-unreachable-gen1-firmware-implementation.md).
- **Phase 8F.6 retires firmware URL-shape compatibility.** The ignored configuration now stores the same Supabase project's HTTPS root, builds reject unsafe or legacy URL shapes without revealing values, and firmware constructs only the three active Gen2 Data API table routes. See the [implementation record](./product/phase8f6-migrate-firmware-supabase-url-to-project-root.md).
- **Phase 8F.7 completes retired-device database discovery and a safety export without mutation.** Live truth, dynamic view exposure, row/dependency order, and a verified protected export of 81,575 proposed retired rows are recorded. Balcony02 is excluded. Deletion remains a separately approved future slice. See the [inventory](./product/phase8f7-retired-device-registry-inventory-and-safety-export.md) and [manifest summary](./product/phase8f7-retired-device-safety-export-manifest-summary.md).
- **Phase 8F.8 retires local device-registry and frontend browser-to-device configuration.** Current frontend source now configures only Balcony02 for Demo, while Customer and Support devices remain authorization-derived from the unchanged database. Retired live rows remain visible until a refreshed and explicitly approved destructive slice. See the [implementation record](./product/phase8f8-retire-local-device-registry-and-frontend-configuration.md).
- **Phase 8F.9 and Phase 8F.10 complete the approved database retirement.** Phase 8F.9 deleted exactly 81,575 proven retired-device rows after refreshing the protected export. Phase 8F.10 attributed and deleted the remaining six development/validation fixtures, retired the empty `sensor_logs` schema/access surface, and revoked unused API-role grants from retained empty `sensor_events`, while preserving Balcony02 and all protected evidence.
- **Phase 8F.11 operationally closes Phase 8F.** The tracked-repository audit finds no remaining executable Gen1 path, current retired identity/configuration dependency, or unresolved schema/access risk. Historical ADRs, phase evidence, executed SQL, manifests, hashes, and protected recovery artifacts remain intact. Phase 8G.1 revises the hosted RMI scale. Phase 8G.2 implements local 30/60-second button programs and retires dormant automatic control without threshold UI. See the [closeout](./product/phase8f11-final-gen1-retirement-audit-and-closeout.md), [Phase 8G.1 record](./product/phase8g1-relative-moisture-index-scale-implementation.md), and [Phase 8G.2 record](./product/phase8g2-local-button-programs-and-automatic-control-retirement.md).
- **Phase 8G.3 is COMPLETE / DEPLOYED / HOSTED-VALIDATED.** P02 is a new Support-only/customer-hidden/Demo-hidden bench identity with an explicit `prototype02-gen2` profile, one each of M02, L04, ST01, WL01, BME280, ADC01, and MUX03, plus a pump-free relay/LED watering simulation. Build, upload, live endpoints, all seven measurements, telemetry/heartbeat, uninterrupted 30/60-second cycles, cancellation, reservoir blocking/cutoff, and active-output network exclusion/recovery passed. Commit `7d3cb6c` was pushed to `main`; Jeremy confirmed the deployed authenticated Support view showed P02 readings and simulation-specific watering events on 2026-08-25. See the [Phase 8G.3 record](./product/phase8g3-prototype02-firmware-update-and-commissioning.md).
- **Phase 8G.3.1 is COMPLETE / LOCALLY VALIDATED.** Environmental reading cards now act as adaptive fuzzy scales: the current condition neighborhood dominates, lower/higher context remains visible, and explicit text continues to carry meaning. Healthy current reservoir evidence collapses to a compact confirmation while refill and uncertain/fault states remain prominent. Tests passed 70/70 with lint, build, diff, desktop, and phone-sized browser validation. No firmware, SQL, query, evidence-state, RMI, reservoir-safety, or watering-authority change occurred. Production deployment and hosted validation are not yet claimed. See the [Phase 8G.3.1 record](./product/phase8g3-1-adaptive-environmental-card-scales.md).
- **Phase 8G.3.2 is COMPLETE / LOCALLY VALIDATED.** `/demo` is temporarily a public live customer-facing example while `/support` remains protected and engineering-oriented. Public/customer expansion is limited to state and latest reading time; Support diagnostics, RMI/raw/formula evidence, sensor identities, provenance, paired-source details, failure counters, manual refresh, and the one-option selector are excluded. Public history is hard-limited to 24 hours with longer choices disabled and labeled `customer only`; public visible polling is 15 minutes. This is a UI boundary over existing intentionally public views, not a server-side rate limiter. Production deployment and hosted validation are not yet claimed. See the [Phase 8G.3.2 record](./product/phase8g3-2-public-live-customer-example.md).
...
| --- | --- | --- | --- |
| **8C.3A — Evidence-state discovery and decisions** | Complete | Inspect freshness, validity, last-good, omission, and query behavior; approve exact policies | No |
| **8C.3B — Evidence-state design closeout** | Complete | Document the approved state model, wording, escalation rules, acceptance criteria, and query guardrails | No |
| **8C.3C — Evidence-state implementation** | Complete | Implement current, stale, invalid, omitted, never-reported, and last-good behavior | Yes—status and evidence wording |
...
| **8D — Watering-event visibility restoration** | Complete / hosted validation | Restored normal and safety-terminal cycles with honest timestamps, duration, type, markers, and history | Regression repair; no watering change |
| **8E — Feels Like and Dew Point** | Complete / hosted validation | Added familiar derived microclimate values with honest paired-evidence rules | Yes—new display-only values |
| **8F — Gen1 risk review, containment, and retirement** | Complete / operationally closed | Retired unsupported frontend/history, firmware/profile/configuration, identity/row, and legacy schema/access surfaces; final repository audit complete | No current Gen1 runtime path |
| **8G.1 — Relative Moisture Index scale** | Complete / locally validated | Revised the provisional hosted observational scale and bands | Display-only; not watering authority |
| **8G.2 — Local button programs / automatic-control retirement** | Complete / B02 quick-validated | 30/60-second release-selected cycles, immediate local cancellation, reservoir safety, and dormant automatic-control removal | Firmware/evidence wording; no threshold UI |
| **8G.3 — Prototype02 Gen2 and commissioning** | Complete / deployed / hosted-validated | New P02 identity/profile, Support-only commissioning, seven-sensor measurement contract, and pump-free relay/LED controller-path proof | New bench unit; no hosted control |
| **8G.3.1 — Adaptive environmental card scales** | Complete / locally validated | Make each environmental card the fuzzy scale and minimize healthy binary reservoir evidence | Display-only; no interpretation or control change |
| **8G.3.2 — Public live customer example** | Complete / locally validated | Use `/demo` for remote customer-presentation feedback without exposing Support UI | Temporary until deterministic Phase 8I Demo |
...
- 🟠 VALIDATION — implemented locally; external/runtime proof remains
...
| ✅ COMPLETE | Phase 8C.1 | Hosted Capability Integration Design | Defined protected capability authority, exact commissioned identity, lifecycle, caching, evidence, and presentation boundaries. |
| ✅ COMPLETE | Phase 8C.2 | Authenticated Support Capability Implementation | Capability-driven Support cards were implemented, deployed, production validated, and operationally closed. |
...
| ✅ COMPLETE | Phase 8D | Watering-Event Visibility Restoration | Normal completions and safety cutoffs are paired and hosted-visible with honest timestamps, durations, types, markers, and history; read-only evidence and local watering authority are preserved. |
...
| ✅ COMPLETE / LOCALLY ACCEPTED | Phase 8G follow-on | Watering-Event Graph Visibility Repair | Fixed plot height, count-preserving dashed lines, centered one-watering labels, centered dense-window count summary, plain-language Watering History wording, and preserved read-only/control boundaries; phone-sized and post-push hosted validation remain follow-ups. |
...
| 🔵 CURRENT | Roadmap follow-on | Brand Foundation and First Marketing Material | Highest executable priority: create a professional business card and the smallest useful provisional logo/visual foundation, with reusable elements that may later inform frontend modernization. |
...
- Derived weather metrics were future work outside Phase 8B.5 and Phase 8B.6; Dew Point and Feels Like were later implemented and closed in Phase 8E.
...
#### Phase 8C.3A–D - Evidence State — COMPLETE

- 8C.3A: inspect freshness, validity, last-good, omission, and query behavior; approve exact policies.
- 8C.3B: close the state model, wording, escalation rules, acceptance criteria, and query guardrails.
- 8C.3C: implement current, stale, invalid, omitted, never-reported, and last-good behavior.
- 8C.3D: validate deployed behavior and close the sequence.

#### Phase 8C.4 - Measurement-Quality Gates — COMPLETE

- Handle implausible values at the correct frontend presentation boundary while preserving original evidence and authority separation.
- Completed provider-specific rules prevent bad values from masquerading as trustworthy readings.

#### Phase 8C.5A - Environmental Presentation Design — COMPLETE

- Approved condition ranges and gardener-facing terminology per measurement.
- Approved measurement-specific card colors, pressure/reservoir treatment, neutral/unavailable behavior, and Support diagnostics boundary.
- Last Good and Last Reliable conditions retain their color through the existing inclusive 50-minute freshness boundary; older evidence is neutral and pill escalation remains unchanged.
...
- **Phase 8D — COMPLETE / HOSTED-VALIDATED:** Watering-Event Visibility Restoration. Commit `406bce9` restored the proven frontend regression without changing watering behavior. Closeout: [`docs/product/phase8d-watering-event-visibility-restoration-closeout.md`](./product/phase8d-watering-event-visibility-restoration-closeout.md).
- **Phase 8E — COMPLETE / HOSTED-VALIDATED:** Feels Like and Dew Point share one exact package-provenance formula/evidence path for cards and chart series. Commit `8d90cec77ec128ee7eeaba304fc24cd5cb3a2453` was pushed to `main` and deployed through the established Cloudflare production path; Jeremy confirmed the visible live features on 2026-08-19. The observation proves deployed feature presence, but byte-for-byte correspondence between the Cloudflare-served bundle and local `HEAD` was not independently proven. No SQL, query, watering, or control-authority change occurred; see [`phase8e-feels-like-and-dew-point-implementation.md`](./product/phase8e-feels-like-and-dew-point-implementation.md).
- **Phase 8F — COMPLETE / OPERATIONALLY CLOSED:** Phase 8F.1–8F.10 retired the bounded unsupported frontend/history paths, obsolete firmware environments and source, legacy URL-shape compatibility, retired frontend/database identities and rows, final development fixtures, and obsolete `sensor_logs` schema/access surface. Phase 8F.11 reconciles current authority, classifies the remaining historical references, and finds no material unresolved Gen1 risk. Records: [`phase8f11-final-gen1-retirement-audit-and-closeout.md`](./product/phase8f11-final-gen1-retirement-audit-and-closeout.md) and its linked evidence chain.
- **Phase 8G.1 — COMPLETE / LOCALLY VALIDATED:** Revised provisional hosted RMI presentation; no firmware or watering authority change.
- **Phase 8G.2 — COMPLETE / B02 QUICK-VALIDATED:** Local 30/60-second release-selected button programs, immediate accepted-press cancellation, reservoir start/cutoff authority, descriptive evidence, and dormant automatic-control retirement. Revision `r3` is uploaded to B02 and passed the user-executed brief start/following-button-stop test; uninterrupted 30- and 60-second completion tests remain deferred until watering is needed. No threshold UI or SQL change. Record: [`phase8g2-local-button-programs-and-automatic-control-retirement.md`](./product/phase8g2-local-button-programs-and-automatic-control-retirement.md).
- **Phase 8G.3 — COMPLETE / DEPLOYED / HOSTED-VALIDATED:** Added and commissioned the new `Prototype02` / `prototype02-gen2` Support bench unit with its own UUID and no retired Prototype01/Bench01 dependency. MUX03, ADC01/M02, L04, BME280, ST01, and WL01 passed locally and in scheduled cloud evidence. Pump-free relay/LED simulation passed uninterrupted 30- and 60-second programs, accepted-press cancellation, empty-reservoir blocking, active-run reservoir cutoff, exact event pairing, and network exclusion/recovery. P02 explicitly reports no pump and no physical water delivery. Commit `7d3cb6c` was pushed and Jeremy confirmed the deployed authenticated Support presentation on 2026-08-25. B02 was not uploaded or cycled. Record: [`phase8g3-prototype02-firmware-update-and-commissioning.md`](./product/phase8g3-prototype02-firmware-update-and-commissioning.md).
- **Phase 8G.3.1 — COMPLETE / LOCALLY VALIDATED:** Environmental cards now present the scale directly with a dominant current fuzzy neighborhood, soft lower/higher context, explicit condition text, and accessible descriptions. Current healthy reservoir evidence is compact; refill and uncertain/fault states remain prominent. No query, SQL, firmware, RMI/evidence interpretation, reservoir safety, or watering authority changed. Production deployment and hosted validation are not yet claimed. Record: [`phase8g3-1-adaptive-environmental-card-scales.md`](./product/phase8g3-1-adaptive-environmental-card-scales.md).
...
- Phase 9A, Local Schedule Foundation: user-visible local schedule configuration and controller-side persistence.
- Phase 9B, Sensor-Assisted Scheduled Watering: fixed-sensor skip/allow logic with conservative fallback behavior.
- This is a secondary product direction, not a continuation required to establish the current working Gen2 product.
- Preserve local firmware ownership of watering decisions and pump shutoff; no hosted command/control.
...
- Phase 8G.2 is the separately approved exception for local button behavior: it does not authorize moisture-triggered automation, an RMI threshold, or hosted watering controls. Its design approval did not by itself authorize deployment/upload; the B02 `r3` upload was separately user-authorized and user-executed.
...
- Phase 8C.1–8C.5B status: COMPLETE at their recorded evidence levels; 8C.5B is committed, pushed, and hosted-validated. Phase 8D watering-event restoration is COMPLETE / HOSTED-VALIDATED in commit `406bce9`. Phase 8E Feels Like/Dew Point is COMPLETE / HOSTED-VALIDATED in commit `8d90cec77ec128ee7eeaba304fc24cd5cb3a2453`; it was pushed, deployed, Jeremy-validated, and operationally closed with the recorded live-feature/no-byte-correspondence proof boundary. Phase 8F is COMPLETE. Phase 8G.1 revised hosted RMI scale is COMPLETE / LOCALLY VALIDATED. Phase 8G.2 local button programs are COMPLETE / B02 QUICK-VALIDATED. Phase 8G.3 Prototype02 Gen2 update and commissioning is COMPLETE / DEPLOYED / HOSTED-VALIDATED in commit `7d3cb6c`. Phase 8G.3.1 adaptive environmental card scales and Phase 8G.3.2 public live customer example are COMPLETE / LOCALLY VALIDATED and pending production deployment/hosted validation. Phase 8G.4 service-side physical sensor identity and lifecycle work is COMPLETE without a firmware update; its remaining firmware payload cleanup is deliberately bundled with the future watering-threshold firmware phase. The watering-event graph visibility repair is COMPLETE / LOCALLY ACCEPTED with phone-sized and post-push hosted validation open. Watering-threshold evidence is the highest product priority but waits for a useful storm-free basket dry-down window. A lightweight brand foundation and first business card are now the highest executable priority, followed by Phase 8H customer adoption/customer-led modernization and Phase 8I deterministic Demo. Phase 9A/9B optional automation is DEFERRED. See [`PROJECT_ROADMAP.md`](./PROJECT_ROADMAP.md) for current ordering and rationale.
...
- Hosted SEN0308 measurement-quality work should remove the obsolete generic raw-ADC display rule and design the presentation boundary directly for the commissioned ADS1115-backed SEN0308 system. Do not adapt or preserve the failed prior moisture path.
...
- Watering chart completeness — RESOLVED by Phase 8D on 2026-08-19 and the graph-visibility follow-on on 2026-08-27: every selected-window watering retains a count-preserving dashed line. One watering is named over its line; dense windows show one centered exact-count summary, and full named evidence remains in Watering History without shrinking the measurement plot. Normal completions and firmware safety stops remain visible rather than being masked. See [`docs/product/phase8d-watering-event-visibility-restoration-closeout.md`](./product/phase8d-watering-event-visibility-restoration-closeout.md) and [`docs/product/watering-event-graph-visibility-repair-phase-slice.md`](./product/watering-event-graph-visibility-repair-phase-slice.md).
