...
| 🔵 CURRENT | Phase 8B | Balcony02 Gen2 Controller Deployment and Endpoint Contract Cleanup | Parent phase: physical buildout and `/measurements` are complete; `/capabilities` is next, followed by `/status` and final endpoint integration. |
...
| 🔵 CURRENT | Phase 8B.3 | Gen2 `/capabilities` Static Contract Cleanup | Make capabilities a stable static/configuration manifest with no sensor scans or measurement reads; preserve installed versus not-installed truth and control-authority boundaries. |
| 🟡 PLANNED | Phase 8B.4 | Gen2 `/status` Nested Diagnostics Contract Cleanup | Create the approved nested human-readable diagnostics snapshot and align heartbeat/hosted diagnostics without changing watering ownership. |
| 🟡 PLANNED | Phase 8B.5 | Gen2 Endpoint Integration and Closeout | Validate `/measurements`, `/capabilities`, and `/status` together; close contract documentation and regression evidence. |
...
#### Phase 8B - Balcony02 Gen2 Controller Deployment and Endpoint Contract Cleanup — CURRENT
...
- Phase 8B.3 Gen2 `/capabilities` Static Contract Cleanup — CURRENT / next slice.
- Phase 8B.4 Gen2 `/status` Nested Diagnostics Contract Cleanup — PLANNED.
- Phase 8B.5 Gen2 Endpoint Integration and Closeout — PLANNED.
- The Phase 8B parent remains current until `/capabilities`, `/status`, and integrated endpoint closeout are finished.
- Balcony02 is built and runtime proven with 3 moisture sensors, 3 light-sensor identities including explicit L01 missing evidence, air context, soil temperature, WL01 reservoir interlock, and the permanent local hold-to-water/test switch. No three-sensor voting logic was introduced.
...
- Phase 8B parent status: CURRENT. Phase 8B.1 Balcony02 physical/runtime prove-out and Phase 8B.2 `/measurements` cleanup/deployment are COMPLETE; Phase 8B.3 `/capabilities` static cleanup is CURRENT; Phase 8B.4 `/status` nested diagnostics cleanup and Phase 8B.5 endpoint integration/closeout are PLANNED.
