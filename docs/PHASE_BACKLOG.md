...
- **Phase 8F.11 operationally closes Phase 8F.** The tracked-repository audit finds no remaining executable Gen1 path, current retired identity/configuration dependency, or unresolved schema/access risk. Historical ADRs, phase evidence, executed SQL, manifests, hashes, and protected recovery artifacts remain intact. Phase 8G Watering-Threshold Presentation is next. See the [closeout](./product/phase8f11-final-gen1-retirement-audit-and-closeout.md).
...
- **Phase 8G — NEXT:** Watering-Threshold Presentation. Show actual configuration and context without changing the threshold or implying scientific recommendation.
