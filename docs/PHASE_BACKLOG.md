...
- Phase 8C.3 Commissioned Measurement Evidence-State and Health Refinement is implemented and locally validated as the bounded Support continuation of Phase 8C.2. It adds the approved last-good, omission, invalidity, freshness, bounded-consecutive-failure, device-active escalation, RMI-source, condition/health separation, and capability-driven Support health policies without adding queries or backend/device changes. Automated validation passed 20/20 tests, lint, build, and `git diff --check`; authenticated local healthy/current presentation review passed. Fault-state behavior remains deterministic-test-supported because healthy sensors were not deliberately faulted. Production deployment/validation is pending. See [`docs/product/phase8c3-commissioned-measurement-evidence-state-and-health-refinement.md`](./product/phase8c3-commissioned-measurement-evidence-state-and-health-refinement.md). Phase 8D remains reserved for Local Schedule UI and Schedule Persistence.
- Phase 8C.4 Deterministic Support Evidence Fault-State Validation is the bounded test-only continuation that directly covers derived-unavailable RMI behavior and verifies fault labels independently from environmental-condition currency. It adds no runtime, query, storage, backend, device, watering, Demo, customer, or visual change. Validation passed 22/22 tests, lint, build, and `git diff --check`; Phase 8C.3 production validation remains separately approved work.
