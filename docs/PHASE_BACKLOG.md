...
- Watering chart completeness — RESOLVED by Phase 8D on 2026-08-19: every selected-window watering cycle now receives a count-preserving marker line, while text labels are limited to the ten newest markers to contain clutter and full named evidence remains in Watering History. Normal completions and firmware safety cutoffs remain visible rather than being masked. See [`docs/product/phase8d-watering-event-visibility-restoration-closeout.md`](./product/phase8d-watering-event-visibility-restoration-closeout.md).
