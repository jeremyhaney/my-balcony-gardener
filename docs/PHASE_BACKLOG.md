...
| ✅ COMPLETE | Phase 8A.1 | Prototype01 installed sensor profile cleanup | Prototype01 now reports expected installed/not-installed sensor truth; no watering authority or frontend card behavior changed. |
| ✅ COMPLETE | Phase 8A | Prototype01 Relative Moisture Index and Card Cleanup | Hosted Support View now uses the accepted display-only gardener moisture index and cleaned card/chart/status behavior. |
...
- Phase 8A.1 status: runtime validated / complete pending commit; Prototype01 installed sensor profile cleanup disables removed VEML6030 and GPIO34 legacy moisture for `bench-proto-gen2`, reports SEN0308/SEN0562 not-installed channel truth, preserves provider/channel metadata, and changes no watering authority or Balcony01 GPIO34/control behavior.
- Phase 8A status: implemented and accepted for closeout pending commit; hosted Support View now derives a display-only gardener Moisture Index from Prototype01 SEN0308 A0 raw ADC, cleans the main cards into Soil/Light/Air Conditions, keeps raw ADC as supporting evidence, treats expected `profile_not_installed` rows as non-warning top-level status evidence, and changes no watering authority.
