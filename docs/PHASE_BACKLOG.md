...
- Phase 8C.1 hosted frontend capability-integration design is documentation-complete in [`docs/product/phase8c1-hosted-frontend-capability-integration-design.md`](./product/phase8c1-hosted-frontend-capability-integration-design.md). The Phase 8C.2 authenticated Support Vertical Slice is implementation-complete and locally validated with shared normalized logical-sensor types, current-effective filtering, per-device session caching, exact logical-series card/history mappings, last-good evidence retention, and no fallback. Jeremy's 2026-08-16 authenticated visual review confirmed Balcony02's eleven-card presentation and the expected zero-capability state for unprovisioned devices. Post-push production validation remains pending; Prototype01 firmware upgrade/provisioning and customer adoption remain later work. The existing Phase 8D remains Local Schedule UI and Schedule Persistence and is unchanged.
