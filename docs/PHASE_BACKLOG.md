...
- **Phase 8F.11 operationally closes Phase 8F.** The tracked-repository audit finds no remaining executable Gen1 path, current retired identity/configuration dependency, or unresolved schema/access risk. Historical ADRs, phase evidence, executed SQL, manifests, hashes, and protected recovery artifacts remain intact. Phase 8G.1 now revises the hosted RMI scale; actual controller-configuration presentation remains next. See the [closeout](./product/phase8f11-final-gen1-retirement-audit-and-closeout.md) and [Phase 8G.1 record](./product/phase8g1-relative-moisture-index-scale-implementation.md).
