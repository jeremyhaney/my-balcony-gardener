...
- Phase 8C.1 hosted frontend capability-integration design is documentation-complete in [`docs/product/phase8c1-hosted-frontend-capability-integration-design.md`](./product/phase8c1-hosted-frontend-capability-integration-design.md). The future Phase 8C.2 authenticated Support Vertical Slice will cover Balcony02, shared normalized logical-sensor types, current-effective filtering, per-device session caching, exact card/history mappings, deterministic evidence states, and no fallback. Customer adoption waits for an authorized customer-visible commissioned device. Phase 8C.2 runtime implementation and deployment remain deferred. The existing Phase 8D remains Local Schedule UI and Schedule Persistence and is unchanged.
