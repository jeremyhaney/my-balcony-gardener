...
- ADR 0024 is accepted as the hosted capability boundary. On 2026-08-14 Jeremy installed `btree_gist` `1.7`, executed the exact committed Phase 8C forward-contract and Balcony02 provisioning statement bodies, and validated the production schema, grants, isolation, and provisioning. `device_capabilities` now holds nine positive current Balcony02 declarations with eleven expected measurement names at the accepted administrative effective instant `2026-08-12T17:03:41Z`; M04/L04/LUX04 and all other devices remain absent. Balcony02 remains Support-visible and customer-hidden, so Support authorization returned nine rows and customer authorization returned zero. Measurement-status and undeclared-measurement Support diagnostics remain deferred pending bounded-query design and production read-only `EXPLAIN`; stored firmware `/capabilities` evidence remains unavailable. Firmware and measurement/heartbeat paths remain evidence and cannot silently create or remove declarations. No frontend consumes the new views, and no hosted or Supabase command/control authority exists. Evidence: [`docs/product/phase8c-hosted-device-capability-production-execution-evidence.md`](./product/phase8c-hosted-device-capability-production-execution-evidence.md).
- Phase 8C.1 capability-integration design is documented in [`docs/product/phase8c1-hosted-frontend-capability-integration-design.md`](./product/phase8c1-hosted-frontend-capability-integration-design.md). The Phase 8C.2 authenticated Support Vertical Slice is implemented and locally validated using `support_device_capabilities`, exact logical-series mappings, current-effective filtering, and a per-device in-memory session cache while preserving measurement/history behavior and forbidding inventory fallback. Automated tests, lint, type checking, and production build passed. Jeremy's authenticated Support visual review on 2026-08-16 confirmed Balcony02's nine commissioned logical sensors produce eleven ordered cards and unprovisioned devices such as Prototype01 correctly show `No commissioned sensors are configured for this device.` Prototype01 firmware upgrade and capability provisioning remain later work; post-push production validation remains pending. Customer adoption, public capability access, assignment, Demo, Gen1, firmware, and Supabase state remain unchanged.
