...
- Hosted display of `watering_events` remains future work and must not create fake rows, introduce Supabase command/control, or change local Water Now behavior.
...
- Supabase remains evidence storage only, pump shutoff remained local, no hosted control path was introduced, and hosted frontend display of `watering_events` remains future work.
