...
- Phase 8G.1 supersedes the Phase 8A formula and condition bands only for current hosted behavior. Current cards and history derive the unclamped index directly as `RMI = 35 + 65 * (11230 - raw_adc) / 3590`, classify the unrounded value at inclusive upper boundaries `35`, `55`, `85`, `140`, and `180`, and round only for presentation. Negative values remain visible Too Dry evidence; values above `100` remain visible; `108`, `121`, and `131` are all Well-watered. RMI is an index, not a percentage, and remains tied to its exact eligible SEN0308 raw source.
