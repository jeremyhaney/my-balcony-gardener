...
- Phase 7L.4 Customer Auth, Garden Membership, and RLS Implementation is locally/browser validated and pending commit, merge, push, deploy, and production/custom-domain validation.
...
- Phase 7L.4 keeps hosted customer/support routes read-only and introduces no Water Now, local ESP32 endpoint calls, Supabase command/control, firmware change, deploy, or firmware upload.
