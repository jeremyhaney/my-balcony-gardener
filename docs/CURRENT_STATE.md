...
- Phase 8F.10 read-only discovery proves the three remaining `sensor_logs` rows are a Phase 4 history-UI development seed and the three remaining `sensor_events` rows are the explicit Phase 5B manual sample-validation set. A verified protected export and separate row/schema-access SQL proposals exist, but no Phase 8F.10 SQL has executed and the live schema/access/counts remain unchanged.
...
- Phase 8F.10 attributes the six retained rows to temporary development/validation identities, proves Balcony02 exclusion by UUID and commissioning timeline, maps both tables' full live schema/access/dependency surface, and prepares two independently approvable proposal-only SQL artifacts. `sensor_logs` has no supported Gen2 consumer and is proposed for drop after exact row deletion. `sensor_events` remains an isolated manual operational log and is proposed for grant cleanup, not table removal. See the [discovery record](./product/phase8f10-final-legacy-row-and-schema-retirement-discovery.md).
