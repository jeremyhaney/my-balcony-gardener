...
- Phase 8C.3 Commissioned Measurement Evidence-State and Health Refinement is implemented and locally validated. Support cards now separate environmental condition from evidence-health pills; preserve timestamped last-good evidence; distinguish invalid, latest-package-omitted, stale, selected-window-empty, derived-unavailable, unsupported-presentation, and dashboard-query states; use 50-minute freshness and 95-minute device-active actionable boundaries; and derive bounded consecutive invalid/omission counts from already-fetched rows. Support Device Status is capability-driven. RMI remains derived from its exact raw ADC source, with formula/engineering evidence excluded from customer views. The slice adds no query, polling, history expansion, storage, SQL, Supabase, firmware, device, or watering change. Automated validation passed 20/20 tests, lint, build, and `git diff --check`; Jeremy's authenticated local Support review confirmed the healthy/current Balcony02 presentation. Fault states were not forced on healthy sensors and remain deterministic-test-supported. Production deployment/validation remains pending. Design and evidence: [`docs/product/phase8c3-commissioned-measurement-evidence-state-and-health-refinement.md`](./product/phase8c3-commissioned-measurement-evidence-state-and-health-refinement.md).
- Phase 8C.4 Deterministic Support Evidence Fault-State Validation directly covers the Phase 8C.3 derived-unavailable RMI policy branch and verifies that invalid, omitted, and stale evidence labels remain separate from environmental-condition currency. It changes no runtime behavior, query, storage, backend, device, watering, Demo, customer, or visual path. Validation passed 22/22 tests, lint, build, and `git diff --check`; the existing chunk advisory remained non-blocking. See [`docs/product/phase8c4-deterministic-support-evidence-fault-state-validation.md`](./product/phase8c4-deterministic-support-evidence-fault-state-validation.md).
