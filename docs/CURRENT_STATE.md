...
- Phase 8F.10 proved the final three `sensor_logs` rows were a Phase 4 history-UI development seed and the final three `sensor_events` rows were the explicit Phase 5B manual sample-validation set, exported them losslessly, and deleted exactly those six rows in the separately approved hash-bound transaction. Both tables remain present and empty; schema, grants, RLS, policies, dependencies, the shared helper, Balcony02, and all 95 protected reservoir batches remain unchanged. The separate schema/access proposal is unapproved and unexecuted.
...
- Phase 8F.10 attributes the six retained rows to temporary development/validation identities, proves Balcony02 exclusion by UUID and commissioning timeline, maps both tables' full live schema/access/dependency surface, and executes only the separately approved exact six-row proposal. `sensor_logs` and `sensor_events` remain present and empty with unchanged access/schema. The separate proposal to drop `sensor_logs` and clean up `sensor_events` API grants remains unapproved and unexecuted. See the [discovery record](./product/phase8f10-final-legacy-row-and-schema-retirement-discovery.md) and [row-deletion execution record](./product/phase8f10-delete-final-legacy-rows.md).
