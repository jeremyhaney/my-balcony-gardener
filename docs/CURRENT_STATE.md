...
- ADR 0024 is accepted as the hosted capability boundary. A narrowed Phase 8C design/review package proposes one positive lifecycle table, one protected customer capability view, one protected Support lifecycle view, separate Balcony02 provisioning rows, validation, and rollback; no SQL has been executed and no capability is provisioned or active. Measurement-status and undeclared-measurement Support diagnostics remain deferred pending bounded-query design and production read-only `EXPLAIN`; stored firmware `/capabilities` evidence remains unavailable. Firmware and measurement/heartbeat paths remain evidence and cannot silently create or remove cards. SQL execution, production validation, frontend work, and Support discrepancy implementation require separate approval.
