...
- ADR 0024 is accepted as the documentation-only hosted capability boundary. Provisioned per-device commissioned capabilities in Supabase will be authoritative for hosted presentation; firmware `/capabilities` and all measurement/heartbeat paths remain runtime evidence and cannot silently create or remove cards. Missing commissioned readings must remain visible. Exact SQL/RLS/grants, provisioning, frontend adapters/cards, history controls, and Support discrepancy UI remain unimplemented and require separate review.
