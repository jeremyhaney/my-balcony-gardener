...
- Phase 8F.10 proved the final three `sensor_logs` rows were a Phase 4 history-UI development seed and the final three `sensor_events` rows were the explicit Phase 5B manual sample-validation set, exported and deleted exactly those six rows, then executed the separately approved schema/access retirement. `sensor_logs` is absent. `sensor_events` remains present, empty, RLS-enabled/not-forced, and has no `anon`, `authenticated`, or `service_role` table privileges. The shared helper and its two current Gen2 policy dependencies, Balcony02, every pre-existing protected row, and all 95 reservoir batches remain unchanged.
- Phase 8F.11 completes the tracked-repository retirement audit and operationally closes Phase 8F. Phase 8G.1 implements the revised provisional hosted RMI display scale. Phase 8G.2 replaces dead-man button watering with 30/60-second local programs and retires dormant automatic-control code/configuration without adding threshold UI. See the [Phase 8F closeout](./product/phase8f11-final-gen1-retirement-audit-and-closeout.md), [Phase 8G.1 record](./product/phase8g1-relative-moisture-index-scale-implementation.md), and [Phase 8G.2 record](./product/phase8g2-local-button-programs-and-automatic-control-retirement.md).
...
- Phase 8G.2 changes only local button watering behavior and evidence wording/provenance; it changes no SQL, schema, indexes, retention, telemetry/heartbeat cadence, device identity, authentication/RLS, hosted control authority, or RMI formula.
- Phase 8G.3 Prototype02 preserves the established MBG Gen2 3.3V-only electrical canon: P02 has no 5V rail, no sensor or relay-control device is supplied from 5V, and no 5V fallback is approved. Prototype01 proved this 3.3V-only mux/ADC/sensor topology, Balcony02 operates on it, and Prototype02 intentionally matches it. P02 troubleshooting must therefore treat 5V as out of scope rather than as a proposed SEN0204, SEN0562, mux, ADC, relay, or other sensor remedy.
- Phase 8G.3 P02 runtime troubleshooting isolated the repeatedly unreliable physical `MUX02`: the Fluke 179 measured a stable `3.294V` at ESP32 3.3V/GND, `3.285V` at the mux, and `3.282V` at ADC01; MUX02 remained undetected at `0x70` with every downstream sensor removed, verified upstream SDA/SCL continuity, and no SDA/SCL short. Jeremy discarded MUX02 and installed newly soldered `MUX03`. MUX03 was detected and passed with ADC01/M02 on channel 0, L04 on channel 1, and BME280 on channel 4. The first post-replacement cloud batch measured at `2026-08-25T17:07:59Z` contained exactly seven valid `read_ok` records: BME280 `79.48 F` / `54.12%` / `1017.12 hPa`, ST01 `78.35 F`, M02 raw ADC `17021`, L04 `30.0 lux`, and WL01 state `1`. Its periodic heartbeat followed at `17:08:03Z`, uptime `902`, Wi-Fi connected at RSSI `-47`, and watering idle. The complete live P02 `/measurements`, `/capabilities`, and `/status` contract validator passed after the replacement.
- Phase 8G.3 is operationally closed. The pump-free relay/LED simulation produced paired uninterrupted 30- and 60-second programs, an immediate accepted second-press cancellation, two empty-reservoir blocked starts, and two active-run WL01 safety cutoffs. The cutoffs paired a 30-second start with `watering_safety_cutoff` / `firmware_safety` / `reservoir_liquid_lost` after one second and included GPIO26 LOW/interlock evidence. Network endpoints were unavailable only while the relay output was active and recovered immediately without reboot. WL01 stayed illuminated continuously during the post-MUX03 uninterrupted 30- and 60-second cycles. Every event explicitly records simulation, relay/LED output, no pump, and no water delivery. Commit `7d3cb6c` was pushed to `main`, and Jeremy confirmed the deployed authenticated Support view showed Prototype02 readings plus `Simulated Button Cycle` and `Simulated Reservoir Safety Stop` evidence on 2026-08-25. B02 was not uploaded or cycled during Phase 8G.3. The dedicated record is [`docs/product/phase8g3-prototype02-firmware-update-and-commissioning.md`](./product/phase8g3-prototype02-firmware-update-and-commissioning.md).
- Phase 8G.3.1 uses adaptive environmental-card backgrounds: lower and higher context remain visible at the edges while the current fuzzy condition neighborhood occupies the dominant field. A compact full-scale pill with an exact current-position marker remains as the `you are here` mini-map. Ordinary and derived environmental cards share the treatment, retain explicit condition/value text, and expose accessible scale/current-condition descriptions. Current `Water Detected` evidence is a compact confirmation; refill, stale, unavailable, invalid, and sensor-trouble reservoir states remain prominent. Local desktop and `390 × 844` responsive validation, 70/70 tests, lint, build, and diff checks passed. No measurement interpretation, RMI boundary, evidence rule, query, SQL, firmware, reservoir safety, or watering authority changed. Production deployment/hosted validation are not yet claimed. See [`docs/product/phase8g3-1-adaptive-environmental-card-scales.md`](./product/phase8g3-1-adaptive-environmental-card-scales.md).
- Phase 8G.3.2 keeps `/support` protected and temporarily presents `/demo` as a public live customer-facing example until deterministic Phase 8I Demo work. The public page retains interpreted readings, scales, trends, quality, and chart controls while hiding Support diagnostics, the one-option device selector, manual refresh/timestamp, RMI/raw ADC/formulas, sensor identities, provenance, paired-source details, and failure counters. Public history is enforced at no more than 24 hours; longer disabled options advertise `customer only`, and public visible-tab polling aligns to the 15-minute telemetry cadence. Expanded public/customer cards show only interpreted state and latest reading time; Support retains engineering detail. This is a UI boundary over intentionally public views, not a raw-data secrecy claim or server-side rate limiter. Local tests, lint, build, diff, and disclosure checks passed; production deployment and hosted validation are not yet claimed. See [`docs/product/phase8g3-2-public-live-customer-example.md`](./product/phase8g3-2-public-live-customer-example.md).
- Supabase `sensor_events` remains an empty isolated manual operational event log; the historical validation fixtures were exported and deleted in Phase 8F.10.
...
- ADR 0025 defines the current local button-program and automatic-control-retirement boundary. ADR 0006 remains historical Phase 5C evidence.
- Phase 5C cooldown firmware was historically uploaded and field validated; the cooldown and executable automatic-control path are now retired from current firmware.
...
- Physical-button release selects a 30-second cycle before `5,000 ms` or a 60-second cycle at/after `5,000 ms`; a valid press while watering cancels immediately, and its release only re-arms. Startup while pressed also requires a release that cannot start a cycle.
- Reservoir absence blocks a selected start immediately. During an active run, GPIO26 must remain continuously LOW for 20 ms before reservoir loss is accepted; any HIGH resets qualification. Persistent loss then shuts off the pump before evidence/network work. Relay LOW initialization/shutoff, queued event evidence, and local/offline authority remain.
- Balcony02 revision `phase8g2-local-button-programs-r2` passed two supervised quick runtime cycles: release started the selected 30-second program, the pump remained active, and a following accepted press stopped it after approximately one second. Each run posted one start and one compatible cancellation terminal. Jeremy subsequently uploaded `phase8g2-local-button-programs-r3` and reported a successful brief start/following-button-stop test on 2026-08-24. Rejected transient LOW reads are silent in `r3` and create no event or network I/O. The uninterrupted 30- and 60-second completion tests remain deliberately deferred until watering is needed.
- Moisture-triggered automatic watering, thresholds, automatic duration/cooldown, and automatic-control gate/sample state are absent from current executable/configuration surfaces. RMI and SEN0308 values remain observational and cannot initiate watering.
- No watering-threshold UI or `/support` redesign was added. Historical automatic-watering events and records remain historical evidence and display-compatible.
...
- Phase 8F.9 deleted exactly 81,575 proven retired-device rows while retaining six then-unmapped rows and all Balcony02 evidence. See the [execution record](./product/phase8f9-delete-retired-device-database-rows.md).
- Phase 8F.10 attributes the six retained rows to temporary development/validation identities, proves Balcony02 exclusion by UUID and commissioning timeline, exports and deletes the exact rows, then retires the obsolete `sensor_logs` schema/access surface and removes unused Data API grants from retained `sensor_events` under a second exact-hash approval. See the [discovery record](./product/phase8f10-final-legacy-row-and-schema-retirement-discovery.md), [row-deletion record](./product/phase8f10-delete-final-legacy-rows.md), and [schema/access execution record](./product/phase8f10-retire-legacy-schema-access.md).
- Phase 8F.11 reconciles current authority with the complete retirement sequence and operationally closes Phase 8F. See the [final audit and closeout](./product/phase8f11-final-gen1-retirement-audit-and-closeout.md).
...
- Phase 8G.1 supersedes the Phase 8A formula and condition bands only for current hosted behavior. Current cards and history derive the unclamped index directly as `RMI = 35 + 65 * (11230 - raw_adc) / 3590`, classify the unrounded value as `≤35 Too Dry`, `>35..≤55 Dry`, `>55..<100 Moist`, `≥100..≤225 Well-watered`, `>225..<235 Very Wet`, and `≥235 Saturated`, and round only for presentation. Negative values remain visible Too Dry evidence; values above `100` remain visible. RMI is an index, not a percentage, and remains tied to its exact eligible SEN0308 raw source.
- Phase 8G.1 documents RMI `55` as the provisional watering-due transition and RMI `35` as already overdue without changing or presenting the controller's actual configured firmware threshold. Raw ADC and formula details remain Support-only. No firmware, watering behavior/authority, database, deployment, device, or production data changed. Product record: [`docs/product/phase8g1-relative-moisture-index-scale-implementation.md`](./product/phase8g1-relative-moisture-index-scale-implementation.md).
...
Physical construction and commissioning are closed. Installed soak and reliability observation; intermittent DS18B20 missing-reading investigation; SEN0308 dry-down/manual-watering measurement-system evaluation; WL01 elevation optimization during natural drawdown; one-gallon reservoir marking/calibration; hydraulic delivered-volume and distribution characterization; bogus-reading/plausibility handling; customer-facing sensor/location metadata; Supabase storage-growth investigation; and a later major website redesign remain open follow-on evidence streams, not construction punch-list. The Gen1-remnant retirement/refactor review is closed through Phase 8F.11. No field prove-out item is claimed complete by this construction closeout.
