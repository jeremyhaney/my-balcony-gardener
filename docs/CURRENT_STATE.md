...
- Phase 8F.11 completes the tracked-repository retirement audit, preserves historical and recovery evidence, finds no remaining executable Gen1 path or current retired identity/configuration/schema dependency, and operationally closes Phase 8F. Phase 8G.1 now implements the revised provisional hosted RMI display scale; the separate Phase 8G presentation of actual controller configuration remains open. See the [Phase 8F closeout](./product/phase8f11-final-gen1-retirement-audit-and-closeout.md) and [Phase 8G.1 implementation record](./product/phase8g1-relative-moisture-index-scale-implementation.md).
