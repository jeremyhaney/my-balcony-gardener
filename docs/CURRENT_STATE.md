...
- Phase 8C.1 capability-integration design is documented in [`docs/product/phase8c1-hosted-frontend-capability-integration-design.md`](./product/phase8c1-hosted-frontend-capability-integration-design.md). It defines the future Phase 8C.2 authenticated Support Vertical Slice using `support_device_capabilities`, exact Balcony02 mappings, current-effective filtering, and a per-device in-memory session cache while preserving measurement/history behavior and forbidding inventory fallback. Customer adoption remains deferred until an authorized customer-visible commissioned device exists. Phase 8C.1 is documentation only and Phase 8C.2 is not implemented: no frontend consumer, public capability access, assignment change, Demo change, or runtime change exists yet.
