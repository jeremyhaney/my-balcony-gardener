...
- Phase 8C.1 capability-integration design and the Phase 8C.2 checkpoint are documented in [`docs/product/phase8c1-hosted-frontend-capability-integration-design.md`](./product/phase8c1-hosted-frontend-capability-integration-design.md). Phase 8C.2 was committed and pushed on `main` as `b1eea01c484d6838e66598f6fd5b0eeae3c2d251` (`Implement Phase 8C.2 Support capabilities`); `HEAD`, `main`, and `origin/main` agreed and the tree was clean before this closeout. The normal post-push hosted update deployed it; no manual deployment occurred. Validation passed 15/15 tests, lint, the TypeScript/Vite production build, and `git diff --check`, with only the existing non-blocking large-chunk advisory. Jeremy approved the 2026-08-16 production smoke test. Directly observed: Balcony02 rendered eleven cards from nine commissioned logical sensors in Light, Air, Water, Soil order: L01/L02/L03; separate Air Temperature, Humidity, and Atmospheric Pressure cards; Reservoir Water; M01/M02/M03; and Soil Temperature, while M04 and L04/LUX04 did not appear; live values, statuses, trends, refresh information, Manual Refresh, Sensor Details, and History functioned; Prototype01 showed the zero-capability state without fabricated cards; and the Demo remained available and materially unchanged. Capability-query failure, future/retired filtering, unknown-measurement fallback, undeclared-measurement exclusion, evidence fixtures, session caching, refresh separation, hidden-tab polling, and concurrency protection remain test/inspection-supported. Result: **Pass**; Phase 8C.2 is operationally closed. Prototype01 provisioning, customer adoption, evidence/health and measurement-quality refinement, Gen1 cleanup, deterministic Demo work, and visual modernization remain future work; Balcony02 remains Support-only/customer-hidden, and Phase 8D remains Local Schedule UI and Schedule Persistence.
- Phase 8C.3 Commissioned Measurement Evidence-State and Health Refinement is production-validated and operationally closed. Support cards separate environmental condition from evidence-health pills; preserve timestamped last-good evidence; distinguish invalid, latest-package-omitted, stale, selected-window-empty, derived-unavailable, unsupported-presentation, and dashboard-query states; use 50-minute freshness and 95-minute device-active actionable boundaries; and derive bounded consecutive invalid/omission counts from already-fetched rows. Jeremy's authenticated 2026-08-17 production Support validation passed for Balcony02: exactly eleven commissioned cards rendered in Light, Air, Water, Soil order; all pills were `Current`; capability-driven quality reported 11/11 readings, 9/9 sensors, 96/96 reports, and all 11 latest readings usable; details, trends, History, and Manual Refresh worked; M04/L04/LUX04 and hosted Water Now were absent; Demo loaded normally; and no console errors appeared. Fault states were not forced and remain deterministic-test-supported. Design and evidence: [`docs/product/phase8c3-commissioned-measurement-evidence-state-and-health-refinement.md`](./product/phase8c3-commissioned-measurement-evidence-state-and-health-refinement.md).
- Phase 8C.4 Deterministic Support Evidence Fault-State Validation is complete in commit `de7e5de8bcc9e1d73bc72333939225210b273843`. It directly covers the Phase 8C.3 derived-unavailable RMI policy branch and verifies that invalid, omitted, and stale evidence labels remain separate from environmental-condition currency. Validation passed 22/22 tests, lint, build, and `git diff --check`; it changed no runtime behavior, query, storage, backend, device, watering, Demo, customer, or visual path. See [`docs/product/phase8c4-deterministic-support-evidence-fault-state-validation.md`](./product/phase8c4-deterministic-support-evidence-fault-state-validation.md).
- Phase 8C.5A Measurement-Quality Gates Discovery and Boundary Design is documentation-approved. It separates device metadata, numeric finiteness, provider electrical possibility, physical plausibility, installation expectations, recent behavior, freshness, environmental interpretation, Support concern, customer eligibility, and firmware control eligibility. Generic raw-ADC trust handling from the failed prior moisture path is obsolete, has no compatibility requirement, and must be removed rather than propagated into Gen2 product logic; the unimported and untested `hostedMeasurementTrust.ts` is not an approved implementation source. Balcony02 measurement-quality work is designed directly for its ADS1115-backed SEN0308 system. Append-only Gen2 evidence and original `valid`/`quality`/`reason` remain unchanged. The preferred later frontend slice uses already-fetched rows and adds no query, SQL, storage flag, backfill, polling, or Disk IO. See [`docs/product/phase8c5a-measurement-quality-gates-discovery-and-boundary-design.md`](./product/phase8c5a-measurement-quality-gates-discovery-and-boundary-design.md).
- Phase 8C.5B Provider-Specific Measurement Eligibility is complete, committed, and pushed. Validation passed 28/28 deterministic tests, lint, build, and `git diff --check`; Jeremy also approved the local site's appearance and behavior. Approved windows are air `0..130 °F` and soil `10..130 °F`; SEN0562 accepts observed `0 lux` darkness and `65535 lux` full-sun clipping with a separate ceiling concern. Provider-ineligible latest rows remain unchanged in Support evidence, cannot supply ordinary current value/condition or trend points, and may fall back to honestly timestamped last-presentation-eligible evidence. The obsolete trust module and prior generic raw-ADC display rule are removed. No query, polling, SQL, storage, firmware, device, watering, manual deployment, or production-validation change occurred. See [`docs/product/phase8c5b-provider-specific-measurement-eligibility-contract.md`](./product/phase8c5b-provider-specific-measurement-eligibility-contract.md).
