...
Status: Approved roadmap sequence; Phases 8D and 8E are complete, hosted-validated, and operationally closed. Phase 8F is active through the approved bounded 8F.1, 8F.2, and 8F.3 frontend-retirement slices. Other slices still require their own discovery, boundary approval, implementation approval, validation, and closeout.
...
- **Phase 8F is active through Phase 8F.3.** Phase 8F.1 retires the unsupported Prototype01 local panel. Phase 8F.2 retires `LiveStats`, local polling/control, and local-control target definitions. Phase 8F.3 routes ordinary and hosted builds through the hosted Gen2 shell and retires Demo's legacy history request plus the frontend `sensor_logs` adapter/types/state/chart/health path. Firmware endpoints/profiles/posting, Supabase storage/policies, hosted Gen2 behavior, and registry entries remain. See [`phase8f1-retire-unsupported-live-measurements-frontend.md`](./phase8f1-retire-unsupported-live-measurements-frontend.md), [`phase8f2-retire-livestats-local-water-now-frontend.md`](./phase8f2-retire-livestats-local-water-now-frontend.md), and [`phase8f3-retire-local-dashboard-sensor-logs-frontend.md`](./phase8f3-retire-local-dashboard-sensor-logs-frontend.md).
