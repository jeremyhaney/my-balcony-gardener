...
Status: Approved roadmap sequence; Phases 8D and 8E are complete, hosted-validated, and operationally closed. Phase 8F is active through the approved bounded 8F.1 and 8F.2 frontend-retirement slices. Other slices still require their own discovery, boundary approval, implementation approval, validation, and closeout.
...
- **Phase 8F is active through Phase 8F.2.** Phase 8F.1 retires the unsupported Phase 7C Prototype01 local frontend panel and its proven-exclusive styling, response types, and request helpers. Phase 8F.2 retires `LiveStats`, selected-target five-second `/logs` polling, browser identity gating, local Water Now, and the exclusively owned local-control target definitions. The shared device registry, `sensor_logs` history path, hosted Gen2 behavior, firmware endpoints/profiles, and Demo behavior remain. See [`phase8f1-retire-unsupported-live-measurements-frontend.md`](./phase8f1-retire-unsupported-live-measurements-frontend.md) and [`phase8f2-retire-livestats-local-water-now-frontend.md`](./phase8f2-retire-livestats-local-water-now-frontend.md).
