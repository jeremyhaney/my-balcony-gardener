...
Status: Approved roadmap sequence; Phases 8D, 8E, and 8F are operationally closed. Phase 8G.1 implements the revised provisional hosted RMI display scale. Phase 8G's separate presentation of actual controller configuration remains open.
...
- **Phase 8G.1 is implemented and locally validated.** Current cards and historical series share the direct unclamped RMI equation and revised gardener-facing bands; Support retains raw ADC/formula evidence. RMI `55` is documented as the provisional watering-due transition, but the frontend gains no watering authority and actual controller-threshold presentation remains open. See the [implementation record](./phase8g1-relative-moisture-index-scale-implementation.md).
